...
class H3DAPIConan(ConanFile):
    name = "h3dui"
    version = "2.4-beta"
    url = "https://github.com/ulricheck/conan-h3dui.git"
    ...
    def source(self):
        repo_url = "https://www.h3dapi.org:8090/UI/trunk/UI/"
        self.run("svn checkout %s source" % repo_url)
        tools.replace_in_file("source/build/CMakeLists.txt", "project( UI )", '''project( UI )
...
''')
    ...
    def package(self):
        ...
        self.copy(pattern='FindUI.cmake', dst="cmake", keep_path=False)
        self.copy(pattern='*', src="source/x3d", dst="examples/ui", keep_path=False)
